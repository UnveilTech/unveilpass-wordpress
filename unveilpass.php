 ...
 * Description: Zero-knowledge password manager for WordPress. Free: password generator. Pro: SSO login, Agent Gateway, 2FA enforcement, security dashboard.
 * Version: 1.0.1
 * Author: UnveilTech
...
define('UNVEILPASS_VERSION', '1.0.1');
...
function unveilpass_generate_password($length = 12, $options = []) {
    $chars = 'abcdefghijklmnopqrstuvwxyz';
    $chars .= 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $chars .= '0123456789';
    if (!isset($options['no_symbols']) || !$options['no_symbols']) {
        $chars .= '!@#$%&*+-=?';
    }
    $password = '';
    $max = strlen($chars) - 1;
...
// Admin bar: password generator button
add_action('admin_bar_menu', function ($wp_admin_bar) {
    $wp_admin_bar->add_node([
        'id' => 'unveilpass-gen',
        'title' => '<span style="display:inline-flex;align-items:center;gap:4px"><img src="' . esc_url(UNVEILPASS_PLUGIN_URL . 'assets/icon-24-white.png') . '" style="height:16px;vertical-align:middle"> Generate Password</span>',
        'href' => '#',
        'meta' => ['onclick' => 'return false;'],
    ]);
}, 100);

// Dashboard widget: password generator
add_action('wp_dashboard_setup', function () {
    wp_add_dashboard_widget('unveilpass_generator', 'UnveilPass — Password Generator', 'unveilpass_generator_widget');
});

function unveilpass_generator_widget() {
    $server_url = rtrim(get_option('unveilpass_server_url', 'https://unveilpass.com'), '/');
    $is_pro = !empty(get_option('unveilpass_agent_key')) || !empty(get_option('unveilpass_client_id'));
    $max_length = $is_pro ? 128 : 12;
    ?>
    <div id="uvp-gen-widget">
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:12px">
            <label>Length: <input type="number" id="uvp-gen-length" value="12" min="4" max="<?php echo $max_length; ?>" style="width:60px"></label>
            <?php if (!$is_pro): ?>
                <span style="color:#DB843D;font-size:12px">(max 12 — <a href="<?php echo esc_url($server_url); ?>" target="_blank">upgrade to Pro</a> for up to 128)</span>
            <?php endif; ?>
        </div>
        <div style="display:flex;gap:8px;margin-bottom:12px">
            <label><input type="checkbox" id="uvp-gen-upper" checked> A-Z</label>
            <label><input type="checkbox" id="uvp-gen-lower" checked> a-z</label>
            <label><input type="checkbox" id="uvp-gen-digits" checked> 0-9</label>
            <label><input type="checkbox" id="uvp-gen-symbols" checked> !@#$%</label>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
            <input type="text" id="uvp-gen-result" readonly style="flex:1;font-family:monospace;font-size:14px;padding:6px 10px;border:1px solid #ddd;border-radius:4px;background:#f9f9f9">
            <button class="button button-primary" onclick="uvpGenerate()">Generate</button>
            <button class="button" onclick="uvpCopy()">Copy</button>
        </div>
        <div id="uvp-gen-strength" style="margin-top:8px;font-size:12px"></div>
    </div>
    <script>
    function uvpGenerate() {
        var len = parseInt(document.getElementById('uvp-gen-length').value) || 12;
        var max = <?php echo $max_length; ?>;
        if (len > max) { len = max; document.getElementById('uvp-gen-length').value = max; }
        if (len < 4) len = 4;
        var chars = '';
        if (document.getElementById('uvp-gen-lower').checked) chars += 'abcdefghijklmnopqrstuvwxyz';
        if (document.getElementById('uvp-gen-upper').checked) chars += 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
        if (document.getElementById('uvp-gen-digits').checked) chars += '0123456789';
        if (document.getElementById('uvp-gen-symbols').checked) chars += '!@#$%&*+-=?';
        if (!chars) chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
        var arr = new Uint32Array(len);
        crypto.getRandomValues(arr);
        var pw = '';
        for (var i = 0; i < len; i++) pw += chars[arr[i] % chars.length];
        document.getElementById('uvp-gen-result').value = pw;
        // Strength indicator
        var score = 0;
        if (len >= 8) score++;
        if (len >= 12) score++;
        ...
        var colors = ['#AA4643','#AA4643','#DB843D','#DB843D','#89A54E','#89A54E'];
        var idx = Math.min(score, 5);
        document.getElementById('uvp-gen-strength').innerHTML = '<span style="color:' + colors[idx] + ';font-weight:600">' + labels[idx] + '</span>';
    }
    function uvpCopy() {
        var el = document.getElementById('uvp-gen-result');
...
// Admin footer: generator inline script for admin bar button
add_action('admin_footer', function () {
    ...
    <script>
    (function() {
        var btn = document.getElementById('wp-admin-bar-unveilpass-gen');
        if (!btn) return;
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            var len = <?php echo $max_length === 12 ? '12' : '20'; ?>;
            var chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%&*+-=?';
            var arr = new Uint32Array(len);
            crypto.getRandomValues(arr);
            var pw = '';
            for (var i = 0; i < len; i++) pw += chars[arr[i] % chars.length];
            prompt('Generated password (Ctrl+C to copy):', pw);
        });
    })();
    </script>
    <?php
...
// REST API endpoint for password generation (no auth required)
add_action('rest_api_init', function () {
    register_rest_route('unveilpass/v1', '/generate', [
        'methods' => 'GET',
        'callback' => function ($request) {
            $is_pro = !empty(get_option('unveilpass_agent_key')) || !empty(get_option('unveilpass_client_id'));
            $max_length = $is_pro ? 128 : 12;
            $length = absint($request->get_param('length')) ?: 12;
            if ($length > $max_length) $length = $max_length;
            if ($length < 4) $length = 4;
            return new WP_REST_Response(['password' => unveilpass_generate_password($length)], 200);
        },
...
function unveilpass_settings_page() {
    ...
    $is_pro = !empty($agent_key) || !empty($client_id);
    ?>
    <div class="wrap">
        ...
        <?php if (!$is_pro): ?>
        <div class="notice notice-info" style="border-left-color:#4572A7">
            <p><strong>Free plan:</strong> Password generator (12 characters max) in the dashboard widget and admin bar. <a href="<?php echo esc_url($server_url); ?>" target="_blank">Upgrade to Pro</a> for SSO, Agent Gateway, 2FA enforcement and unlimited password length.</p>
        </div>
        <?php endif; ?>
        ...
        <form method="post" action="options.php">
            ...
            <h2 class="title">Sign in with UnveilPass (OIDC SSO) <span style="background:#DB843D;color:#fff;font-size:11px;padding:2px 8px;border-radius:4px;vertical-align:middle;font-weight:600">Pro</span></h2>
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="description">Automatically create WordPress accounts for new UnveilPass users (as Subscriber).</span>
                    </td>
                </tr>
            </table>

            <h2 class="title">Agent Gateway <span style="background:#DB843D;color:#fff;font-size:11px;padding:2px 8px;border-radius:4px;vertical-align:middle;font-weight:600">Pro</span></h2>
            <p>Fetch credentials from your vault programmatically using <code>unveilpass_get_credential($entry_id)</code>.</p>
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description">Format: uvp_agent_xxxx. Create in Manager Console &rarr; Agent Gateway.</p>
                    </td>
                </tr>
            </table>

            <h2 class="title">Security <span style="background:#DB843D;color:#fff;font-size:11px;padding:2px 8px;border-radius:4px;vertical-align:middle;font-weight:600">Pro</span></h2>
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        ...
                        <span class="description">Administrators must sign in via UnveilPass SSO. Direct password login blocked for admins.</span>
                    </td>
                </tr>
            </table>
        ...
        <hr>
        <p style="color:#888;font-size:12px">UnveilPass WordPress Plugin v<?php echo UNVEILPASS_VERSION; ?> &mdash; <a href="https://unveilpass.unveiltech.com/integrations.html" target="_blank">Documentation</a> &mdash; <a href="https://unveilpass.unveiltech.com/blog/unveilpass-integrations-guide.html" target="_blank">Integration Guide</a></p>
    </div>
    <?php
}
...
// ============================================================
// OIDC SSO — SIGN IN WITH UNVEILPASS (PRO)
// ============================================================
...
add_action('init', function () {
    ...
    if (!$user) {
        wp_die('No WordPress account found for ' . esc_html($email) . '. Ask your administrator to enable auto-creation.', 'Authentication Error', ['back_link' => true]);
    }
